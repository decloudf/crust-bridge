...
export async function ethTxParser(
  ...
  try {
    ...
    if (method !== 'transfer' || !_.isArray(inputs) || inputs.length !== 2) {
      logger.warn('  ↪ ☠️  Not CSM transfer transaction');
      return null;
    }
    ...
    if (to !== cruBurnAddr) {
      logger.info(`  ↪ ☠️  Not CSM burn transaction: ${to} != ${cruBurnAddr}`);
      return null;
    }
    const from = tx.from;
    ...
    logger.info(
      `  ↪ ✨  Got legal CSM claim transaction: {'from': ${from}, 'to': ${to}, 'amount': ${amount.toString()}}`
    );
...
/**
 * Claim and mint CSM in Crust Shadow
